...
1. If you haven't installed Pelican, follow the instructions to [install Pelican](../install.mdx).

2. Run your Pelican Origin server. You may run other Pelican servers, but in this documentation we will use the Origin server for demonstration. For details about serving a Pelican Origin, refer to [Federating Your Data](../federating-your-data.mdx) page.

    ```bash copy
    pelican origin serve -v /tmp/stash:/test-namespace -f "osg-htc.org"
    ```

3. Go to the URL for Web UI and confirm it's working. The default URL is `https://localhost:8444`. You should be directed to the Pelican login page.
...
By default, Pelican protects the Prometheus endpoint by asking for a credential to verify the user accessing the endpoint. It reads `Bearer <token>` from the `Authorization` header during any HTTPS request to `/api/v1.0/prometheus` and its subpaths. The recommended way to issue this credential is to generate a Pelican API key with the `monitoring.query` scope and pass it to Grafana.

1. Run the following command on your Pelican server. It requires the server's admin token (passed via `--token` or auto-discovered from the standard locations).

    * Replace `<server-web-url>` with the URL to your Pelican server Web UI. By default, it's `https://localhost:8444`. This value must match the server's `Server.ExternalWebUrl`.

    * Replace `--expiration` with your desired expiration time in RFC3339 format. The example below sets a far-future expiration.

    ```bash
    $ pelican apikey generate --scopes "monitoring.query" --expiration "2038-01-19T03:14:07Z" --server <server-web-url>
    ```

2. Copy the API key value printed by the command. Treat it as a credential.
...
4. Fill in `Authorization` for the **Header** field and `Bearer <api-key>` for the **Value** field with your generated API key. Note that there is a whitespace between `Bearer` and the key value.

> **Note**: An API key with only the `monitoring.query` scope is sufficient for Grafana to query the PromQL endpoint at `/api/v1.0/prometheus`. To scrape the raw `/metrics` endpoint from an external Prometheus instance, use the `monitoring.scrape` scope instead. See [Scrape Pelican Metrics with an External Prometheus](./prometheus.mdx#scrape-pelican-metrics-with-an-external-prometheus) for details.
...
That's it! Feel free to modify the dashboard to tailor to your needs. For a list of available Prometheus metrics to query, please refer to the [previous section](./prometheus.mdx).
