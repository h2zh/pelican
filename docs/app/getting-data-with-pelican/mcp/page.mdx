...
Once configured, your AI assistant has access to four Pelican tools:
...
Authenticates to a protected Pelican namespace using OAuth device flow.
When accessing a protected namespace, use this tool to get a verification URL that you can click to authorize access.
After authorization, the token is cached for future operations.
...
**How it works**:
1. The tool initiates an OAuth device flow and returns a verification URL
2. Click the URL to open it in your browser
3. Complete the authorization in your browser
4. The token is automatically cached for future operations
...
Let me initiate the authentication process.
[Uses pelican_auth tool]

To authenticate, please visit this URL and approve the request:
