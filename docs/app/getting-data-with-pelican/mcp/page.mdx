...
The Pelican Model Context Protocol (MCP) server enables AI assistants like Claude Desktop, VS Code Copilot, and other MCP-compatible tools to download files, inspect metadata, and list directories from Pelican federations. This allows you to use natural language to interact with Pelican data directly from your AI-powered development environment.
...
- You have an MCP-compatible AI assistant installed (Claude Desktop or VS Code with GitHub Copilot)
...
Claude Desktop natively supports MCP servers, making it easy to integrate Pelican.
...
After saving the configuration file, completely quit Claude Desktop (not just close the window) and reopen it.

**macOS**: Press `Cmd+Q` then relaunch
**Windows**: Right-click the system tray icon and select "Quit", then relaunch
**Linux**: Use your window manager's quit command, then relaunch

### 4. Verify Connection

In Claude Desktop, look for an MCP indicator (often a hammer/tool icon) that shows connected servers. You should see "pelican" listed as a connected MCP server.
...
- [Claude Desktop Documentation](https://claude.ai/desktop) - More on using MCP with Claude
