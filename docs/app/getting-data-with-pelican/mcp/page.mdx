...
- Work with both public and protected namespaces
...
Some namespaces require authentication. To download from protected namespaces using MCP:

### Generate a Token

First, generate an authentication token using the Pelican CLI:

```bash copy
pelican object get pelican://<federation-url>/<protected-namespace>/<path> <destination>
```

This will guide you through authentication and store the token in your token wallet. See [Getting Protected Objects](../client/page.mdx#get-a-protected-object-from-your-federation) for details.

### Provide Token to AI Assistant

When requesting protected data from your AI assistant, you can either:

**Option 1: Let the assistant find the token automatically**

The MCP server will use tokens from your Pelican token wallet if available:

```
Download the protected file pelican://my-federation.org/protected/data.txt to ~/Downloads/
```

**Option 2: Provide a token explicitly**

If you have a token file, you can specify it in your prompt:
...
pelican://osg-htc.org/ospool/PROTECTED/ligo/data/H1/strain.hdf5
