...
Claude Code (the web-based version of Claude) natively supports MCP servers, making it easy to integrate Pelican.
...
After saving the configuration file, refresh your browser tab or restart Claude Code to load the new MCP server configuration.

### 4. Verify Connection

In Claude Code, look for an MCP indicator (often a tools or server icon) that shows connected servers. You should see "pelican" listed as a connected MCP server.
