...
var (
	...
	currentIssuerKeyDir atomic.Value
...
func getCurrentIssuerKeyDir() string {
	if val := currentIssuerKeyDir.Load(); val != nil {
		return val.(string)
	}
	return ""
}

func setCurrentIssuerKeyDir(dir string) {
	currentIssuerKeyDir.Store(dir)
}
...
func migratePrivateKey(newDir string) error {
	...
	// Rename the existing private key file using the naming pattern in new directory
	fileName := fmt.Sprintf("%s.pem", key.KeyID())
...
func GeneratePEM(dir string) (jwk.Key, error) {
	...
	// Rename the file using its key.KeyID()
	newFilename := fmt.Sprintf("%s.pem", key.KeyID())
	newKeyPath := filepath.Join(dir, newFilename)
	if err := os.Rename(keyPath, newKeyPath); err != nil {
		return nil, errors.Wrap(err, "failed to rename private key file with KeyID")
	}
...
func loadIssuerPrivateKey(issuerKeyDir string) (jwk.Key, error) {
	...
	currentIssuerKeyDir := getCurrentIssuerKeyDir()
	if currentIssuerKeyDir != issuerKeyDir {
		log.Debugf("The private keys dir generated by IssuerKey param has changed from '%s' to '%s'", currentIssuerKeyDir, issuerKeyDir)
		ResetIssuerJWKPtr()
		ResetIssuerPrivateKeys()
		setCurrentIssuerKeyDir(issuerKeyDir)
	}

	// Replace the leaf directory or file name with a new dir /issuer-keys
...
func loadIssuerPublicJWKS(existingJWKS string, issuerKeyDir string) (jwk.Set, error) {
	jwks := jwk.NewSet()
	if existingJWKS != "" {
		var err error
	...
	if key == nil {
		...
		loadedKey, err := loadIssuerPrivateKey(issuerKeyDir)
		if err != nil {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to generate public key from file %v", issuerKeyDir)
	}
...
func GetIssuerPrivateJWK() (jwk.Key, error) {
	...
	if key == nil {
		...
		newKey, err := loadIssuerPrivateKey(issuerKeyDir)
		if err != nil {
...
func GetIssuerPublicJWKS() (jwk.Set, error) {
	...
	issuerKeyDir := param.IssuerKey.GetString()
	return loadIssuerPublicJWKS(existingJWKS, issuerKeyDir)
}
